...
function findBestWildcardCombination(nonJokers: any[], jokers: any[], totalCards: number): HandEvaluation {
  ...
  // Check for Royal Flush (5 cards with 10, J, Q, K, A in same suit)
  if (totalCards === 5) {
    const royalRanks = ['10', 'J', 'Q', 'K', 'A'];
    const nonJokerSuits = Array.from(new Set(nonJokers.map((c: any) => c.suit)));

    for (const suit of nonJokerSuits) {
      const suitNonJokers = nonJokers.filter(c => c.suit === suit);
      const suitNonJokerRanks = suitNonJokers.map(c => c.rank);

      const areAllRoyal = suitNonJokerRanks.every(r => royalRanks.includes(r));
      if (areAllRoyal) {
        const missingRoyals = royalRanks.filter(r => !suitNonJokerRanks.includes(r));
  ...
  if (totalCards >= 4) {
    ...
    if (fourKindResult.success) {
      return {
        type: 'Four of a Kind',
        bonus: HAND_BONUSES.FOUR_OF_A_KIND,
        scoringIndices: [...fourKindResult.indices, ...jokers.map(j => j.originalIndex)]
      };
    }
  }
  ...
  if (totalCards >= 3) {
    ...
    if (threeKindResult.success) {
      return {
        type: 'Three of a Kind',
        bonus: HAND_BONUSES.THREE_OF_A_KIND,
        scoringIndices: [...threeKindResult.indices, ...jokers.map(j => j.originalIndex)]
      };
    }
  }
  ...
  if (totalCards >= 4) {
    ...
    if (twoPairResult.success) {
      return {
        type: 'Two Pair',
        bonus: HAND_BONUSES.TWO_PAIR,
        scoringIndices: [...twoPairResult.indices, ...jokers.map(j => j.originalIndex)]
      };
    }
  }
  ...
  if (totalCards >= 2) {
    ...
    if (onePairResult.success) {
      const scoringIndices = [...onePairResult.indices, ...jokers.map(j => j.originalIndex)];
      // Special case: if we have more than 2 cards, we should only return the 2 that form the pair
      // But for simple implementation, we'll keep the logic consistent.
      return {
        type: 'One Pair',
        bonus: HAND_BONUSES.ONE_PAIR,
...
function checkStraightWithJokers(nonJokers: any[], jokerCount: number): { success: boolean, indices: number[] } {
  if (nonJokers.length === 0) return { success: jokerCount >= 5, indices: [] };
...
function checkFourOfAKindWithJokers(nonJokers: any[], jokerCount: number): { success: boolean, indices: number[] } {
  if (nonJokers.length === 0) return { success: jokerCount >= 4, indices: [] };

  const rankCounts: Record<string, number[]> = {};
  nonJokers.forEach((c: any) => {
    if (!rankCounts[c.rank!]) rankCounts[c.rank!] = [];
    rankCounts[c.rank!].push(c.originalIndex);
  });

  const bestRank = Object.keys(rankCounts).sort((a, b) => {
    if (rankCounts[b].length !== rankCounts[a].length) return rankCounts[b].length - rankCounts[a].length;
    return RANK_VALUES[b] - RANK_VALUES[a];
  })[0];

  const maxCount = rankCounts[bestRank].length;
  if (maxCount + jokerCount >= 4) {
    return { success: true, indices: rankCounts[bestRank] };
  }
  return { success: false, indices: [] };
}

// Helper: Check for Full House with jokers
function checkFullHouseWithJokers(nonJokers: any[], jokerCount: number): { success: boolean, indices: number[] } {
  if (nonJokers.length === 0) return { success: jokerCount >= 5, indices: [] };
  ...
  if (sortedRanks.length >= 2) {
    jokersNeeded = Math.max(0, 3 - rankCounts[sortedRanks[0]].length) + Math.max(0, 2 - rankCounts[sortedRanks[1]].length);
    if (jokersNeeded <= jokerCount && nonJokers.length + jokerCount >= 5) {
      return { success: true, indices: [...rankCounts[sortedRanks[0]], ...rankCounts[sortedRanks[1]]] };
    }
  } else if (sortedRanks.length === 1) {
    const count = rankCounts[sortedRanks[0]].length;
    if (count >= 3) jokersNeeded = 2; // Need pair
    else if (count === 2) jokersNeeded = 3; // Need triple
    else jokersNeeded = 4; // Single card, need 2 more for triple + 2 for pair

    if (jokersNeeded <= jokerCount && nonJokers.length + jokerCount >= 5) {
      return { success: true, indices: rankCounts[sortedRanks[0]] };
    }
  }
...
function checkThreeOfAKindWithJokers(nonJokers: any[], jokerCount: number): { success: boolean, indices: number[] } {
  if (nonJokers.length === 0) return { success: jokerCount >= 3, indices: [] };
...
function checkTwoPairWithJokers(nonJokers: any[], jokerCount: number): { success: boolean, indices: number[] } {
  if (nonJokers.length === 0) return { success: jokerCount >= 4, indices: [] };
  ...
  const pairs = sortedRanks.filter(r => rankCounts[r].length >= 2);
  const singles = sortedRanks.filter(r => rankCounts[r].length === 1);

  if (pairs.length >= 2) return { success: true, indices: [...rankCounts[pairs[0]], ...rankCounts[pairs[1]]] };
  if (pairs.length === 1 && jokerCount >= 2) return { success: true, indices: rankCounts[pairs[0]] }; // Jokers form second pair
  if (pairs.length === 1 && singles.length >= 1 && jokerCount >= 1) return { success: true, indices: [...rankCounts[pairs[0]], rankCounts[singles[0]][0]] }; // Joker pairs with single
  if (singles.length >= 2 && jokerCount >= 2) return { success: true, indices: [rankCounts[singles[0]][0], rankCounts[singles[1]][0]] }; // Jokers pair with two singles

  return { success: false, indices: [] };
}

// Helper: Check for One Pair with jokers
function checkOnePairWithJokers(nonJokers: any[], jokerCount: number): { success: boolean, indices: number[] } {
  if (nonJokers.length === 0) return { success: jokerCount >= 2, indices: [] };
